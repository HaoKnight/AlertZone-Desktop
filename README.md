...
- 托盘／macOS 菜单栏菜单提供可勾选的“启用声音”和“弹窗位置”入口；勾选启用声音，取消勾选则静音
- 声音开关与告警设置同步，重新开启时恢复此前的提示音类型
...
- 选择“仅提示音提醒”时自动禁用主页和托盘菜单中的“弹窗位置”入口
...
当前发布版本为 **1.2.3**。`build.py` 会将该版本写入 Windows 可执行文件版本资源，
以及 macOS 的 `CFBundleShortVersionString` 和 `CFBundleVersion`。

GitHub Release 正文使用项目根目录的 `release_notes.md`。每次发布前，更新
`build.py` 和 `src/AlertZone_Desktop.py` 中的 `APP_VERSION`，同步更新日志中的
`## v版本号` 和本次更新内容，并保留四个下载链接占位符：

- `{{WINDOWS_X64_URL}}`：Windows 64 位安装版
- `{{WINDOWS_X64_PORTABLE_URL}}`：Windows 64 位便携版
- `{{MACOS_ARM64_URL}}`：macOS Apple M 芯片版
- `{{MACOS_X64_URL}}`：macOS Intel 芯片版

将更新日志与源码一起提交后，可运行 **Build + Release AlertZone Desktop** 一次完成
构建和发布；也可先运行 **Build AlertZone Desktop**，再在
**Release AlertZone Desktop** 中填写成功构建的 `build_run_id`。
发布使用该构建提交中的更新日志，上传并验证四个安装包后，自动将占位符替换为
GitHub 返回的实际下载链接，再公开发布。更新日志缺失、版本不一致或占位符不匹配时，
发布会停止；已有版本不会被覆盖。
...
点击窗口关闭按钮时，Desktop 默认直接进入后台静默运行，不弹出操作选择或后台提示。
完全退出程序请使用任务栏／托盘菜单中的“退出”。若系统托盘不可用，则保留主窗口并提示原因，避免无法恢复或退出。
